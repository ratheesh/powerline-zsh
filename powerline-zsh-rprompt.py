...
class Color:
    ...
    VIRTUAL_ENV_BG = 5 #35  # a mid-tone green
    VIRTUAL_ENV_FG = 15
...
class Powerline:
    ...
    def draw(self):
        return (self.reset + ''.join((s[0].draw(self, s[1]) for s in zip(self.segments, self.segments[1:] + [None]))))
...
def add_jobs_segment(powerline):
    ...
    if num_jobs > 0:
        powerline.append(Segment(powerline, ' %d ' % num_jobs, Color.JOBS_FG, Color.JOBS_BG))
